...
input int MAperiod = 12, MAshift = 0;
...
int OnInit() {
  ...
  MaFastInd = iMA(Symbol(), Period(), Fast_MA_Period, MAshift, Fast_MA_Method,
                  Fast_MA_Applied);
  MaSlowInd = iMA(Symbol(), Period(), Slow_MA_Period, Slow_MA_Shift,
                  Slow_MA_Method, Slow_MA_Applied);

  // --- Half Trend ---
  // MaHighInd = iMA(Symbol(), Period(), MAperiod, MAshift, MAmode, MApriceEnd);
  // MaLowInd =
  //     iMA(Symbol(), Period(), MAperiod, Slow_MA_Shift, MAmode, MApriceStart);
...
void OnTick() {
  ...
  double lowprice_i = iLow(Symbol(), Period(),
                           iLowest(Symbol(), Period(), seriesStart, Factor, 1));
  double highprice_i = iHigh(
      Symbol(), Period(), iHighest(Symbol(), Period(), seriesEnd, Factor, 1));

  // --- Half Trend ---
  //     CrossDownDone = false;
  //     CrossUpDone = false;
  //
  // if((_MaHigh[1] < lowprice_i) && (iClose(Symbol(),Period(),1) <
  // iLow(Symbol(),Period(),2)))
  //
  //    {
  //      CrossDownDone =true;
  //    }
  //  if(((_MaLow[1] > highprice_i)) && (iClose(Symbol(),Period(),1) >
  //  iHigh(Symbol(),Period(),2)))
  //    {
  //      CrossUpDone =true;
  //    }
    ...
    if (!PositionSelectByTicket(PTick[t])) {
      ...
      if (CrossDownDone) {
        if (result[0] == "12" && result[1] == "Buy") {
          ...
          AllPos.TrySetValue(PTick[t],
                             "20*Buy*" + result[2] + "*0" + "*" + result[4]);
        }
        if (result[0] == "20" && result[1] == "Sell") {
          AllPos.TrySetValue(PTick[t],
                             "22*Sell*" + result[2] + "*0" + "*" + result[4]);
        }
      }
      ...
      if (CrossUpDone) {
        if (result[0] == "12" && result[1] == "Sell") {
          ...
          AllPos.TrySetValue(PTick[t],
                             "20*Sell*" + result[2] + "*0" + "*" + result[4]);
        }
        if (result[0] == "20" && result[1] == "Buy") {
          AllPos.TrySetValue(PTick[t],
                             "22*Buy*" + result[2] + "*0" + "*" + result[4]);
        }
      }
      ...
      if (result[0] == "22") {
        ...
        if (PosProfit >= ((double)StepsStringSplit[0] * StayPosRatioMin *
                          ((double)StepsStringSplit[1] / Point()) / (-10))) {
          TradeClass.PositionClose(PTick[t]);
          AllPos.Remove(PTick[t]);
        }

        // double trtrtrt = (double)StepsStringSplit[0] * StayPosRatioMax *
        // ((double)StepsStringSplit[1]/Point())*10;
        if (PosProfit < ((double)StepsStringSplit[0] * StayPosRatioMax *
                         ((double)StepsStringSplit[1] / Point()) / (-10) * 2)) {
          // TradeClass.PositionClose(PTick[t]);
          // AllPos.Remove(PTick[t]);
          MqlTradeResult Newres;
          Newres = OrderPlace(
              result[1], 33, 0,
              StartLot * ((double)StepsStringSplit[0] * StayPosRatioMax), "");

          AllPos.TrySetValue(PTick[t], "23*" + result[1] + "*" + result[2] +
                                           "*" + Newres.deal + "*" + result[4]);
        }
      }
      ...
      if (result[0] == "23") {
        ...
        if (PosProfit < 2 * ((double)StepsStringSplit[0] * StayPosRatioMax *
                             ((double)StepsStringSplit[1] / Point()) / (-10))) {
          MqlTradeResult Newres;
          Newres = OrderPlace(
              result[1], 44, 0,
          ...
          AllPos.TrySetValue(PTick[t], "24*" + result[1] + "*" + result[2] +
                                           "*" + result[3] + "*" + result[4]);
          AllPos.TrySetValue((ulong)result[3],
                             "34*" + result[1] + "*" + result[2] + "*" +
                                 Newres.deal + "*" + result[4]);
        }
      }
...
MqlTradeResult OrderPlace(string PosType, int magicNo, double CalcTP,
                          double Lot, string orderComment) {
  if (PosType == "Buy") {
...
string CalcStep(string PosType) {
  ...
  //     {
  //      SumATRBig += MathAbs(iOpen(Symbol(),ATRBigTimeFrame,j) -
  //      iClose(Symbol(),ATRBigTimeFrame,j));
  //
  //     }
  //
  //    for(int k=1;k<ATRSmallCount+1;k++)
  //     {
  //      SumATRSmall += MathAbs(iOpen(Symbol(),ATRSmallTimeFrame,k) -
  //      iClose(Symbol(),ATRSmallTimeFrame,k));
  //
  //     }
  //
  //     return    "2$"+((string)((((3*(SumATRBig/ATRBigCount)) -
  //     (SumATRSmall/ATRSmallCount)))));

  //  Comment((string)SumATRBig+"^^^^^"+(string)SumATRSmall+"^^^^^"+(string)LastHighDist+"^^^^^");
  //     if ( (((3*(SumATRBig/ATRBigCount)) - (SumATRSmall/ATRSmallCount)))  >
  //     LastHighDist)
  //           {
...
void PositionFetch() {
  ...
  for (int i = 0; i < positions; i++) {
    ...
    if (ticket !=
        0) // if the order was successfully copied into the cache, work with it
    {
      ...
      if (!AllPos.ContainsKey(ticket)) {
        if (type == POSITION_TYPE_BUY)
          AllPos.Add(ticket, ((string)magic_number + "*" + "Buy" + "*" +
                              ordcomment + "*" + "0" + "*" + price_open));
        if (type == POSITION_TYPE_SELL)
          AllPos.Add(ticket, ((string)magic_number + "*" + "Sell" + "*" +
                              ordcomment + "*" + "0" + "*" + price_open));
      }
    }
  }
